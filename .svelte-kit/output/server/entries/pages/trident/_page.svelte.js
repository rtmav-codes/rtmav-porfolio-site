...
function Menu($$renderer, $$props) {
  ...
  if (open) {
    $$renderer.push("<!--[0-->");
    $$renderer.push(`<div class="font-mono bg-black svelte-1i27u80"><!--[-->`);
    const each_array = ensure_array_like(["Home", "Surgical Theater", "Wallet Guard", "Speak2Me"]);
    for (let i = 0, $$length = each_array.length; i < $$length; i++) {
      let link = each_array[i];
      $$renderer.push(`<a${attr("href", links[i])} style="text-decoration:none" class="svelte-1i27u80"><p class="svelte-1i27u80">${escape_html(link)}</p></a>`);
    }
...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    function $$render_inner($$renderer3) {
      head("3krc6j", $$renderer3, ($$renderer4) => {
